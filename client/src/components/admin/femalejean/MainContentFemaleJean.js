...
export default class MainContentFemaleJean extends React.Component{
    ...
    addProduct=(name,image,price,size)=>{
        axios.post('http://localhost:8080/femalejean',{
            name,
            image,
            price,
            size
    ...
    updateProduct = (id,name,image,price) => {
        axios.patch(`http://localhost:8080/femalejean/${id}`,{
            name,
            image,
            price
        },{
    ...
    deleteProduct = (id) => {
        Swal.fire({
            ...
            if (result.value) {
                axios.delete(`http://localhost:8080/femalejean/${id}`,{
                    id
                   },{
                       headers:{
                           token: window.localStorage.getItem('admin_token')
                ...
                <div className="content-table">
                    ...
                    </div>
                    {
                        ...
                        this.state.products.map((product)=>{
                            return <ProductRowFemaleJean updateIsEditting={this.updateIsEditting}  deleteProduct={this.deleteProduct} key={`product_id_${product.id}`} product={product}/>
                        })
                        :<EmptyFemaleJean/>
                    }
                </div>
