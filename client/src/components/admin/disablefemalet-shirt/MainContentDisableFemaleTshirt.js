...
import {EmptyDisableFemaleTshirt} from './EmptyDisableFemaleTshirt'
...
export default class MainContentDisableFemaleTshirt extends React.Component{
    ...
    getData = () =>{
        axios.get("http://localhost:8080/disablefemalet-shirt").then(res=>{
            console.log(res);
            this.setState({
                products: res.data
            })
    ...
    deleteDisableProduct = (id) => {
        Swal.fire({
            ...
            if (result.value) {
                axios.delete(`http://localhost:8080/disablefemalet-shirt/${id}`,{
                    id
                   },{
                       headers:{
                ...
                <div className="content-table">
                    ...
                    </div>
                    {
                        ...
                        this.state.products.map((product)=>{
                            return <ProductRowDisableFemaleTshirt deleteDisableProduct={this.deleteDisableProduct} addOnSaleProduct={this.addOnSaleProduct} key={`product_id_${product.id}`} product={product}/>
                        })
                        :<EmptyDisableFemaleTshirt/>
                    }
                </div>
            </main>
        </>
